...
// let 제품명 = prompt('가격 입력')
// let 제품명 = prompt('제조사 입력')
// // 2. 객체{ 속성명 : 속성값 , 속성명 : 속성값 } 여러 자료들을 속성명으로 구분
// //  vs 배열[ 값, 값, 값 ] 여러자료들을 인덱스 구분
// let product = { 제품명, 가격, 제조사 } //{제품명}
// // 입력받은 제품명이 저장된 변수를 객체내 속성값으로 저장 // 모든 자료들은 변수/상수 저장 가능
...
let 이름 = prompt( '이름 입력' )
let member = {'id' : 아이디, 'passsword' : 비밀번호, 'name' : 이름}
// + 만일 members(배열)내 입력받은 id가 존재하면 , 주의할점 : 배열내 객체의 특정값을 조회 -> 배열내 객체를 하나씩 조회
for(let index = 0 ; index <= members.length - 1 ; index++){ //
    let 객체 = members[ index ] ; // member배열내 index번째의 객체 꺼내기
    if(객체.id == member.id) { //만약에 index번쨰 객체의 아이디와 입력받은 아이디와 같다면}
        console.log('[중복]이미 등록된 아이디 입니다.')
        break; // 가장 가까운 반복문 탈
    } 
    // 주의할점 : 반복문이 끝날때까지 모든 배열내 객체 조회, 첫번재 객체내 아이디가 같지 않더라도 뒤에 있는 객체가 같을 수 있다.
}
