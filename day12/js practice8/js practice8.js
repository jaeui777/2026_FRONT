...
function productAdd( ){
    ...
    let image = document.querySelector('.image').files[0] // + 첨부파일은 .files[0] 속성에서 첨부파일의 첫번째 자료 가져오기 
        ...
        if(category == 'disabled') {
            alert('카테고리 선택해주세요') ;
            return; //* 코드의 흐름이 return 만나면 아래 코드는 생략하고 강제함수종료
        }
    // 2. 입력받은 값들을 객체화 , 중간검사 : console.log( )를 이용한 변수/자료 출력
    ...
    let object = { ccode : category , pname : name , pprice : price , 
                    // 만약에 첨부파일 선택이 없으면 (undefined) 기본이미지https://placehold.co/100x100() 있으면 선택한 첨부파일 가상URL 생성
                    pimg : image == undefined ? ''  // 입력받은 이미지 정보를 가상URL 생성해주는 라이브러리
